...
from models.hook1 import FedHook
from utils.attack_methods import gaussian_attack,label_filp
...
class LocalUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, indd=None,peer_type=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        ...
        # self.loss_fun=
        if 'femnist' in args.dataset or 'sent140' in args.dataset: 
            self.ldr_train = DataLoader(DatasetSplit(copy.deepcopy(dataset), np.ones(len(dataset['x'])),name=self.args.dataset), batch_size=self.args.local_bs, shuffle=True)
        else:
            if self.peer_type == 'attacker' and args.attack_type == 'label_attack':
                posion_data=label_filp(DatasetSplit(dataset, idxs), self.source_class, self.target_class)
                self.ldr_train = DataLoader(posion_data, batch_size=self.args.local_bs, shuffle=True)
            else:
                self.ldr_train = DataLoader(DatasetSplit(copy.deepcopy(dataset), idxs), batch_size=self.args.local_bs, shuffle=True)
    ...
    def train(self, net, w_glob_keys,last=False, dataset_test=None, ind=-1, idx=-1, lr=0.1,net_glob=None,lam=None):
        # def backward_hook(module, gin, gout):
        #     # print("hooker working")
        #     # print(gin)
        #     # print(gout)
        #     self.grad_backward_in.append(gin)
        #     self.grad_backward_out.append(gout)
        # #modules=list(self.model.named_children())
        # def forward_hook(module, fin, fout):
        ...
        if last: ##如果为最后一轮
            if self.args.alg =='fedavg' or self.args.alg == 'prox':
                ...
                local_eps = 5
                w_glob_keys = []
            else: ##fedrep为此选项，计算本地总轮次（rep训练轮次+head）
                local_eps =  max(10,local_eps-self.args.local_rep_ep)
        head_eps = local_eps-self.args.local_rep_ep##求头部训练的轮次
        epoch_loss = []
        num_updates = 0
        if 'sent140' in self.args.dataset:
            hidden_train = net.init_hidden(self.args.local_bs)
        for iter in range(local_eps):
            ...
            batch_loss = []
            grad_bp_out = [[], [], []]
            #对于每个用户，数据分割为50*10,batch_size=10,batch_idx=50
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                if 'sent140' in self.args.dataset:
                    input_data, target_data = process_x(images, self.indd), process_y(labels,self.indd)
                    if self.args.local_bs != 1 and input_data.shape[0] != self.args.local_bs:
                    ...
                    if (self.args.attack_type == 'backdoor') and (self.peer_type == 'attacker'):
                    ...
                    if (self.args.attack_type == 'backdoor') and (self.peer_type == 'attacker'):
                        pdata = images.clone()
                        ptarget = labels.clone()
                        keep_idxs = (labels == self.source_class)
                        pdata = pdata[keep_idxs]
                        ptarget = ptarget[keep_idxs]
                        pdata[:, :, -x_offset:, -y_offset:] = backdoor_pattern
                        ptarget[:] = self.target_class
                        images = torch.cat([images, pdata], dim=0)
                        labels = torch.cat([labels, ptarget], dim=0)

                    if self.args.hook == 1 and iter==local_eps-1 :
                        grad = FedHook(net)
                        loss,grad_backward_out,grad_backward_in=grad.grad_bp(images,labels,self.args,self.loss_func,optimizer)
                        #print(grad_backward_out)
                        for i in range(3):
                            grad_bp_out[i].append(grad_backward_out[i])
                        optimizer.zero_grad()
            ...
            if self.args.hook == 1 and iter==local_eps-1 :
                epoch_grad.append([sum(grad_bp_out[k]) / len(grad_bp_out[k]) for k in range(3)])
            if done:
                break
            ...
            if self.args.hook == 1 and iter == local_eps - 1:
                epoch_grad.append([sum(grad_bp_out[k]) / len(grad_bp_out[k]) for k in range(3)])
        if self.args.attack_type == 'gaussian' and self.peer_type == 'attacker':
            update,flag=gaussian_attack(net.state_dict(),malicious_behavior_rate = 1)
            if flag == 1:
                self.performed_attacks+=1
            net.load_state_dict(update)
        return net, sum(epoch_loss) / len(epoch_loss), self.indd
        #         else:
        #             images, labels = images.to(self.args.device), labels.to(self.args.device)
        #             net.zero_grad()
        #             # net.fc3.register_forward_hook(forward_hook)
        #             # net.fc3.register_backward_hook(backward_hook)
        #             #grad=FedHook(net)
        #             #loss=grad.vision(2images,labels,self.args,self.loss_func,optimizer)
        #             log_probs = net(images)
        #             #hook = net.register_backward_hook(hook=self.backward_hook)
        #             # net_layers = net.children()
        #             # for layer in net_layers:
        #             #     layer.register_backward_hook(hook=backward_hook)
        #
        #             loss = self.loss_func(log_probs, labels)
        #
        #             loss.backward()
